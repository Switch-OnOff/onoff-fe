  ...
  <div class="filters-wrap">
    ...
    <div class="filters-bar">
      ...
      <div class="deal-row">
        ...
        <!-- 액션 버튼 (상세필터/초기화) -->
        <div class="actions">
          <button class="btn-detail bodyMedium14px" type="button" @click="openSheet = true">
            상세필터
          </button>
          <button class="btn-reset bodyMedium14px" type="button" @click="resetAll">
            초기화
          </button>
        </div>
      </div>
      ...
      >
        ...
        <div class="sheet-body">
          <RangeField
            label="권리금"
            :unit="'원'"
            v-model:from="draft.premium.min"
            v-model:to="draft.premium.max"
            :min="PREMIUM.min"
            :max="PREMIUM.max"
            :step="PREMIUM.step"
          />
          ...
          <RangeField
            label="매매가"
            :unit="'원'"
            v-model:from="draft.salePrice.min"
            v-model:to="draft.salePrice.max"
            :min="SALE.min"
            :max="SALE.max"
            :step="SALE.step"
          />
          ...
          <RangeField
            label="보증금"
            :unit="'원'"
            v-model:from="draft.deposit.min"
            v-model:to="draft.deposit.max"
            :min="DEPOSIT.min"
            :max="DEPOSIT.max"
            :step="DEPOSIT.step"
          />
          ...
          <RangeField
            label="월세"
            :unit="'원'"
            v-model:from="draft.rent.min"
            v-model:to="draft.rent.max"
            :min="RENT.min"
            :max="RENT.max"
            :step="RENT.step"
          />
          ...
          <RangeField
            label="면적(평)"
            :unit="'평'"
            v-model:from="draft.areaPyeong.min"
            v-model:to="draft.areaPyeong.max"
            :min="AREA.min"
            :max="AREA.max"
            :step="AREA.step"
          />
...
/* ===== 자동 적용: 전 항목 변경 시 곧바로 내보내기(디바운스 + 시그니처 가드) ===== */
let autoTimer = null
let lastSig = ''
...
function scheduleAutoApply() {
  ...
  autoTimer = setTimeout(emitChange, 80)
}

/* 개별 변경 감지 (대/중분류/세그먼트 + 모든 범위) */
...
const RangeField = defineComponent({
  ...
  data() {
    return {
      localFrom: this.from ?? this.min,
      localTo:   this.to   ?? this.max,
    }
  },
  watch: {
  ...
  methods: {
    ...
    clamp(){
      if (this.localFrom > this.localTo) {
        const t = this.localFrom; this.localFrom = this.localTo; this.localTo = t
      }
  ...
  <div class="range-field">
    <div class="field-head">
      ...
      <span class="field-val bodyMedium14px">
        {{ nf(localFrom) }}{{ unit }} ~ {{ nf(localTo) }}{{ unit }}
      </span>
    </div>

    <div
      class="dual-range"
      :style="{ '--from': pct(localFrom) + '%', '--to': pct(localTo) + '%'}"
    >
      <input
        type="range"
        :min="min"
...
/* 세그먼트 트랙: 안쪽 패딩으로 thumb/버튼 공통 기준 맞춤 */
.segment-track{
  position: relative;
  display: grid;
...
/* 버튼: 트랙 안쪽(30px)에 딱 맞춰 중앙정렬 */
.seg-btn{
  height: 100%;               /* = 36px - padding(3px*2) = 30px */
  display: flex;
  align-items: center;        /* 세로 중앙 */
  justify-content: center;    /* 가로 중앙 */
  padding: 0 12px;
  margin: 0;
  line-height: 1;             /* 폰트 라인박스 영향 제거 */
...
.actions{
  ...
  gap: 8px;               /* 버튼 사이 간격 */
  width: clamp(180px, 28vw, 260px); /* 화면에 따라 180~260px */
}

.btn-detail,
.btn-reset{
  flex: 1 1 0;            /* 두 버튼을 동일 너비로 */
  width: auto;            /* 내용 너비 대신 부모 채우기 */
  height: 36px;
  padding: 0 12px;
  border-radius: 10px;
  border: 1px solid var(--color-lightgray);
...
.sheet-handle {
  ...
  width: 40px; height: 4px; border-radius: 9999px; background: var(--color-lightgray);
}
.sheet-head h4 { grid-column: 1 / -1; text-align: center; color: var(--color-black); padding-top: 8px; }
.x {
  position: absolute; right: 0; top: 4px;
  width: 36px; height: 36px; border: 1px solid var(--color-lightgray);
  background: var(--color-white); border-radius: 10px; cursor: pointer;
}
...
.dual-range::before {
  ...
  height: 4px; border-radius: 999px; background: var(--color-lightgray);
}
.dual-range::after {
  content: ""; position: absolute; left: var(--from); right: calc(100% - var(--to)); top: 15px;
...
.dual-range input[type="range"]::-moz-range-thumb {
  ...
  background: var(--color-white); border: 1px solid var(--color-lightgray);
