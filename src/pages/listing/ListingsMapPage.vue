<template>
  <div class="map-page">
    ...
    <SearchBar
      class="top-search"
      :placeholder="'지역·주소·장소·역명 등을 검색하세요'"
      @search="onSearch"
    />
...
import SearchBar from '@/pages/listing/components/SearchBar.vue'
  ...
  try {
    ...
    clusterer = new w.kakao.maps.MarkerClusterer({
      ...
      styles: [{
        ...
        background: 'var(--color-primary-80)', color: 'var(--color-white)',
        borderRadius: '12px', textAlign: 'center', lineHeight: '40px',
        border: '2px solid var(--color-white)', fontFamily: "'Pretendard', sans-serif",
        fontWeight: 700, fontSize: '16px',
      }],
    })

    // 클러스터 클릭 → 바텀시트
    w.kakao.maps.event.addListener(clusterer, 'clusterclick', async (cluster) => {
      const ms = cluster.getMarkers()
      const ids = ms.map((m) => m.__item?.id).filter(Boolean)
      try {
        const results = await Promise.all(ids.map((id) => fetchCardByLocationId(id)))
...
function rebuildMarkers() {
  ...
  markers = propertyLocations.value.map((item) => {
    ...
    w.kakao.maps.event.addListener(m, 'click', async () => {
      try {
        ...
        if (!card) return alert('이 위치에 연결된 매물을 찾지 못했습니다.')
        openOverlay(card, m)
      } catch (e) {
  ...
  else markers.forEach((m) => m.setMap && m.setMap(map))
}
...
async function fetchImageByPropertyId(propertyId) {
  ...
  try {
    const res = await axios.get(`http://localhost:8080/api/posts/card/${propertyId}`)
    const data = res?.data?.data
    const b64 = Array.isArray(data) ? data?.[0]?.representativeImage : data?.representativeImage
    if (!b64) return null
...
/* 바텀시트 제어 + 상세 이동 */
function closeClusterSheet() { isBottomSheetOpen.value = false; clusterItems.value = [] }
...
/* 검색: 장소 키워드 → 주소 지오코딩 순으로 시도 */
async function onSearch(keyword) {
  if (!keyword || !map) return
  const w = window
  const places = new w.kakao.maps.services.Places()
  const geocoder = new w.kakao.maps.services.Geocoder()

  // 1) 장소 키워드 검색
  const place = await new Promise((resolve) => {
    places.keywordSearch(keyword, (data, status) => {
      if (status === w.kakao.maps.services.Status.OK && data?.length) resolve(data[0])
      else resolve(null)
    }, { size: 5 })
  })

  if (place) {
    const latlng = new w.kakao.maps.LatLng(Number(place.y), Number(place.x))
  ...
  const addr = await new Promise((resolve) => {
    geocoder.addressSearch(keyword, (result, status) => {
      if (status === w.kakao.maps.services.Status.OK && result?.length) resolve(result[0])
      else resolve(null)
    })
  })

  if (addr) {
    const latlng = new w.kakao.maps.LatLng(Number(addr.y), Number(addr.x))
    map.setCenter(latlng)
    map.setLevel(3)
    updateURL()
  } else {
    alert('검색 결과가 없습니다. 다른 키워드를 입력해 주세요.')
  }
}
...
onBeforeUnmount(() => {
  ...
  markers.forEach((m) => m.setMap && m.setMap(null))
  markers = []; clusterer = null; markerImage = null; map = null
})
</script>
...
/* ListingsMapPage.vue - <style scoped> 안에 추가 */
.top-search :deep(.search-wrap){
  position: absolute;
  top: calc(env(safe-area-inset-top, 0px) + 8px);
  width: 90%;
  left: 50%;
  right: auto;              /* SearchBar.vue의 left/right를 무력화 */
...
.overlay-container{ pointer-events: auto; touch-action: auto; }
</style>
